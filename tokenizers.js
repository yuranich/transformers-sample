...
function isCcCfCoCs(char) {
    const code = char.codePointAt(0);
  
    // Control characters (Cc): U+0000..U+001F, U+007F..U+009F
    ...
    for (const [start, end] of cfRanges) {
      if (code >= start && code <= end) {
        return true;
      }
    }
  
    // Private use characters (Co): U+E000..U+F8FF, U+F0000..U+FFFFD, U+100000..U+10FFFD
    if (
    ...
    // Surrogate code points (Cs): U+D800..U+DFFF
...
class ByteLevelPreTokenizer extends PreTokenizer {
    ...
    constructor(config) {
        ...
        this.pattern = /'s|'t|'re|'ve|'m|'ll|'d| ?[A-Za-z]+| ?[0-9]+| ?[^\sA-Za-z0-9]+|\s+(?!\S)|\s+/g;
